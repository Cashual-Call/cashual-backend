    ...
    socket.on('disconnect', async (reason) => {
      ...
      if (userId) {
        try {
          ...
          console.log(`[WebSocket] Cleaned up user ${userId} from search queues on disconnect`);
        } catch (error) {
          console.warn(`[WebSocket] Failed to cleanup user ${userId} from search queues:`, error);
        }
      }
    });
  });
