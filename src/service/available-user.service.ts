...
export class AvailableUserService {
	...
	async cleanupDataTypeInconsistencies() {
		try {
			...
			if (userKeyType === "set") {
				console.log(
					`[${this.searchType}] Converting users key from SET to ZSET`,
				);
				...
				console.log(
					`[${this.searchType}] Converted ${users.length} users from SET to ZSET`,
				);
			}
			...
			for (const key of interestKeys) {
				...
				if (keyType === "set") {
					console.log(
						`[${this.searchType}] Converting interest key ${key} from SET to ZSET`,
					);
	...
	async addUser(userId: string, username = "", interests: string[] = []) {
		...
		if (username) {
			const existingUserIds = await redis.smembers(
				`users:${this.searchType}:index:username:${username}`,
			);
	...
	async updateUserHeartbeat(userId: string) {
		...
		await redis.hset(
			`user:${this.searchType}:${userId}`,
			"lastHeartbeat",
			Date.now().toString(),
		);
	}

	async cleanupInactiveUsers(timeoutMs: number = 30000) {
		// 30 seconds timeout
		const currentTime = Date.now();
		const availableUserIds = await redis.zrange(
			`users:${this.searchType}`,
			0,
			-1,
		);
	...
	async getUserInterests(userId: string): Promise<string[]> {
		return redis.zrange(`user_interests:${this.searchType}:${userId}`, 0, -1);
	}
