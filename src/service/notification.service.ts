...
export class NotificationService {
	...
	async sendNotification(
		...
		try {
			...
			if (saveToDb) {
				...
				logger.debug(
					`Notification saved to database with ID: ${dbNotification.id}`,
				);
			}
		...
		try {
			...
			const pushPromises = subscriptions.map(async (subscription) => {
				try {
					...
					logger.error(
						`Error sending push to subscription ${subscription.id}:`,
						error,
					);
			...
			logger.error(
				`Failed to send push notifications to user ${userId}:`,
				error,
			);
			throw error;
		}
	...
	async clearAllNotifications(userId: string): Promise<number> {
		try {
			...
			logger.error(
				`Failed to clear all notifications for user ${userId}:`,
				error,
			);
	...
	async bulkActions(
		...
		try {
			...
			logger.info(
				`Bulk ${action} performed on ${result.count} notifications for user ${userId}`,
			);
			...
			logger.error(
				`Failed to perform bulk ${action} for user ${userId}:`,
				error,
			);
	...
	async broadcastNotification(
		...
		try {
			...
			logger.info(
				`Broadcast notification sent to ${results.successCount}/${userIds.length} users`,
			);
			return results;
		} catch (error) {
	...
	async scheduleNotification(
		...
		try {
			...
			logger.info(
				`Notification scheduled for user ${userId} at ${scheduledFor}`,
			);
