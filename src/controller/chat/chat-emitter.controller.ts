...
export class ChatEmitterController {
	...
	private setupSubscriptions() {
		...
		subClient.subscribe(RedisHash.CHAT_MESSAGES, (err, count) => {
			...
			console.log(
				`[ChatEmitter] Subscribed to ${count} channel(s) for chat messages`,
			);
		});
		...
		subClient.subscribe(RedisHash.CHAT_ROOMS, (err, count) => {
			...
			console.log(
				`[ChatEmitter] Subscribed to ${count} channel(s) for chat rooms`,
			);
		});
	...
	private async handleChatMessage(message: Message) {
		...
		console.log(
			`[ChatEmitter] Emitting message to room '${message.roomId}' (${socketsInRoom.length} sockets in room)`,
		);
	...
	private handleRoomEvent(event: RoomEvent) {
		...
		if (!event.roomId || event.roomId.trim() === "") {
			console.error(
				"Cannot emit room event: roomId is missing or empty",
				event,
			);
			return;
		}
