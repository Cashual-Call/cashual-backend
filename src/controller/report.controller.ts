...
export class ReportController {
		...
		try {
			...
			if (!reporterId || !reportedUserId || !reason) {
				return res.status(400).json({
					error:
						"Missing required fields: reporterId, reportedUserId, and reason are required",
				});
			}
	...
	getReportsByReportedUser = async (req: Request, res: Response) => {
		try {
			...
			// Try to get from cache first
			const cachedReports = await redis.get(
			...
			const reports =
				await this.reportService.getReportsByReportedUser(reportedUserId);
			...
			res
				.status(500)
				.json({ error: "Failed to fetch reports for reported user" });
		}
	};
