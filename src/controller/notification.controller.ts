...
export class NotificationController {
	...
	subscribeToPush = async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const pushSubscription =
				await this.notificationService.addPushSubscription(
					userId,
					subscription,
					userAgent,
				);
	...
	getNotificationPreferences = async (
		...
		try {
			...
			const preferences =
				await this.notificationService.getNotificationPreferences(userId);
	...
	bulkActions = async (req: Request, res: Response): Promise<void> => {
		try {
			...
			const result = await this.notificationService.bulkActions(
				userId,
				action,
				notificationIds,
			);
	...
	scheduleNotification = async (req: Request, res: Response): Promise<void> => {
		try {
			const { userId, type, title, message, data, priority, scheduledFor } =
				req.body;
			...
			if (!userId || !type || !title || !message || !scheduledFor) {
				res.status(400).json({
					success: false,
					message:
						"Missing required fields: userId, type, title, message, scheduledFor",
				});
				return;
			}
	...
	private getNotificationTypeDescription(type: NotificationType): string {
		const descriptions: Record<NotificationType, string> = {
			...
			[NotificationType.ACHIEVEMENT_UNLOCKED]:
				"Sent when achievements are unlocked",
			[NotificationType.SUBSCRIPTION_EXPIRING]:
				"Sent when subscription is about to expire",
