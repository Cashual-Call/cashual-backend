...
export class PaymentController {
  ...
  static async handlePaymentSuccess(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.log(`Payment success for user ${req.user?.email || req.user?.username}:`, {
        transaction,
        amount,
        planType,
  ...
  static async handlePaymentError(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Log payment error
      console.error(`Payment error for user ${req.user?.email || req.user?.username}:`, {
        transaction,
        errorMessage,
        planType,
   ...
   */
  static async createDodoPaymentLink(req: Request, res: Response): Promise<void> {
    try {
      // Validate request body
      ...
      console.log(`DodoPayments link created for user ${user.email || req.user?.email || ""}:`, {
   ...
   */
  static async getDodoStaticCheckoutUrl(req: Request, res: Response): Promise<void> {
    try {
      // Validate request body
      const { planType } = stripePaymentIntentSchema.parse(req.body);
  ...
  private static async handleDodoPaymentSuccess(payload: any) {
    ...
    if (!paymentId) {
      console.error("Missing payment_id or subscription_id in DodoPayments webhook:", payload);
      return;
    }
    ...
    try {
      ...
      // Calculate subscription end date
      const proEnd = calculateProEndDate(planType as "week" | "month" | "annual");
